...
cluster-based: 选择核心节点: 每个普通节点相邻于至少一个 Core 节点
...
IntServ 更适合 Real-time 应用，因为它能 reserve resources end-to-end ，提供 guaranteed 服务 with bounded delay 和 no loss
...
过订阅 is defined by the ratio of ports facing downwards vs. ports facing upwards
	问题：交换机上行 get heavily 负载 由于 poor 二分带宽
...
clos网络中的交换机是crossbar switch。crossbar提供 non-blocking switching, 但需要很多交交叉点，所以成本高、难拓展. Clos用多级小型crossbar来构建大规模switch with 低成本
阻塞概率P=[1-(1-p)^2]^m, p是内部占用率，p=集中倍数*平均外部占用率
小line occupancy 导致 negligible 阻塞概率。交换complexity能被减少 by 集中流量 at 第一阶段

控制与转发解耦如何改进网络管理？controller决定路由和policies，交换机只负责按规则转发。这样网络管理更centralized、更易配置，也更容易编程和更新策略
将网络功能与硬件分离如何促进网络服务交付？防火墙、NAT、负载均衡等功能可以作为软件运行在general-purpose服务器上，而不是dedicated设备。这样服务部署、扩展、升级和迁移都更快，成本更低，灵活性更高
