...
* **Example:** **Photodiode** (PIN, avalanche).
    * 光电二极管 
        * In a reverse-biased **P-N junction detector**, **electron–hole** pairs generated by the incident optical energy increases the **carrier density**. 
        * 在反向偏置的 **P–N 结探测器**中，入射光能会产生**电子-空穴对**并增加**载流子浓度**
...
![image-20251216174729194](https://media.opennet.top/i/2025/12/17/2fpfk8-0.png)
...
- **Zn** -> Zn oxide, releasing electrons.
...
$$
**放电过程：** 负极的 **Zn** 发生氧化，释放电子并生成氧化锌（~~ZnO~~ 更正，图中写的是二氧化锌）。正极的二氧化锰 MnO₂ 被还原并消耗电子。电子从负极流向正极，从而产生电能。

- **Advantages:** Low cost, long shelf life
- **Disadvantages:** Non-rechargeable.

> 下图中的化学式疑似有误。Zn 不太可能变成 ZnO₂，而应该是 ZnO
...
- **Mechanism:** Electrostatic. Stores charge via a **double laye**r formed at the **electrode-electrolyte interface** (Helmholtz layer).
    - 静电。通过在电极-电解质界面形成的**双电层**（亥姆霍兹层）存储电荷。
